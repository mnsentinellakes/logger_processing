    ...
    function(x){
      ...
      monthcalc = unlist(
        lapply(
          ...
          function(y){
            c(
              nrow(monthdat[which(year(monthdat$Date_Time) == y & month(monthdat$Date_Time) == x & monthdat$FlagVis != 'F'),]),
              nrow(monthdat[which(year(monthdat$Date_Time) == y & month(monthdat$Date_Time) == x),])
            )
          }
        )
      )
...
colnames(monthtotals) = gsub(".monthtib","",colnames(monthtotals))
...
### Appendix
```{r qc thresholds functions, include = TRUE,echo=FALSE,results='as.is',ft.align = 'left'}
qcthresholddisp = function(qcthresholds,paranum){
  # paranum = 1
  dataparameters = unique(qcthresholds$Logger_Type)
  
  if (length(dataparameters) >= paranum){
    qcthresholds = qcthresholds[which(qcthresholds$Logger_Type == dataparameters[paranum]),]
    
    if (length(unique(qcthresholds$Level)) > 1){
      qcthreshresults = NULL
      for (i in unique(qcthresholds$Level)){
        # i = unique(qcthresholds$Level)[1]
        
        if (!is.na(unique(qcthresholds$Z_1))){
          lvl = paste0("Level: ",i,"\n(",unique(qcthresholds$Z_1[which(qcthresholds$Level == i)])," - ",unique(qcthresholds$Z_1[which(qcthresholds$Level == i)]),")")
        }else{
          lvl = paste0("Level: ",i)
        }
        
        qcthreshresultsrow = tibble(
          "Level" = lvl,
          "Metric" = c("Gross","Gross","Gross","Gross","Spike","Spike","Rate of Change","Rate of Change","Flat","Flat","Flat"),
          "Threshold" = c("Max Fail","Min Fail","Max Suspect","Min Suspect","Max","Min","SD Number","SD Period","Max","Min","Tolerance"),
          "Value" = unlist(sapply(1:11,function(x){qcthresholds$Value[which(qcthresholds$Level == i)][x]})),
          "Units" = unlist(sapply(1:11,function(x){qcthresholds$Units[which(qcthresholds$Level == i)][x]}))
        )
        
        qcthreshresults = rbind(qcthreshresults,qcthreshresultsrow)
      }
      
      parametername = dataparameters[paranum]
      names(qcthreshresults)[1] = parametername
      
      qcthreshtable = flextable(qcthreshresults) %>%
        border_remove() %>%
        merge_v(j = parametername) %>%
        merge_v(j = "Metric") %>%
        autofit() %>% 
        report_theme()
      
      flextable_to_rmd(qcthreshtable)
      
      
    }else{
      
      qcthreshresults = tibble(
        "Metric" = c("Gross","Gross","Gross","Gross","Spike","Spike","Rate of Change","Rate of Change","Flat","Flat","Flat"),
        "Threshold" = c("Max Fail","Min Fail","Max Suspect","Min Suspect","Max","Min","SD Number","SD Period","Max","Min","Tolerance"),
        "Value" = qcthresholds$Value,
        "Units" = qcthresholds$Units
      )
      
      qcthreshtable = flextable(qcthreshresults) %>%
        border_remove() %>%
        add_header_row(values = dataparameters[paranum],colwidths = 4) %>%
        merge_v(j = "Metric") %>%
        autofit() %>% 
        report_theme()
...
:::: {style="display: flex;"}
::: {}
```{r qc threshholds 1, include = parachunkdisp[1],eval=parachunkdisp[1],echo = FALSE,results='asis',ft.align = 'left'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 1
)
```
```{r qc threshholds 3, include = parachunkdisp[3],eval=parachunkdisp[3],echo = FALSE,results='asis',ft.align = 'left'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 3
)
```
```{r qc threshholds 5, include = parachunkdisp[5],eval=parachunkdisp[5],echo = FALSE,results='asis',ft.align = 'left'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 5
)
```
```{r qc threshholds 7, include = parachunkdisp[7],eval=parachunkdisp[7],echo = FALSE,results='asis',ft.align = 'left'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 7
)
```
```{r qc threshholds 9, include = parachunkdisp[9],eval=parachunkdisp[9],echo = FALSE,results='asis',ft.align = 'left'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 9
)
```
```{r qc threshholds 11, include = parachunkdisp[11],eval=parachunkdisp[11],echo = FALSE,results='asis',ft.align = 'left'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 11
)
...
:::
::: {}
```{r qc threshholds 2, include = parachunkdisp[2],eval=parachunkdisp[2],echo = FALSE,results='asis',ft.align = 'right'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 2
)
```
```{r qc threshholds 4, include = parachunkdisp[4],eval=parachunkdisp[4],echo = FALSE,results='asis',ft.align = 'right'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 4
)
```
```{r qc threshholds 6, include = parachunkdisp[6],eval=parachunkdisp[6],echo = FALSE,results='asis',ft.align = 'right'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 6
)
```
```{r qc threshholds 8, include = parachunkdisp[8],eval=parachunkdisp[8],echo = FALSE,results='asis',ft.align = 'right'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 8
)
```
```{r qc threshholds 10, include = parachunkdisp[10],eval=parachunkdisp[10],echo = FALSE,results='asis',ft.align = 'right'}
qcthresholddisp(
  qcthresholds = thresh,
  paranum = 10
)
